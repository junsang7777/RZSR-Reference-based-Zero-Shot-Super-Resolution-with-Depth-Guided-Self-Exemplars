...
def My_random_augment(img,
                      leave_as_is_probability=0.1,
                      no_scale_probability=0.3,
                      min_scale = 0.5,
                      max_scale = 1.0,
                      crop_size = 128
                      ):
    ...
    if random_chooser < leave_as_is_probability:
        mode = 'leave_as_is'
    elif leave_as_is_probability < random_chooser < leave_as_is_probability + no_scale_probability :
        mode = 'no_scale_rot_filp'
    else:
        mode = 'scale_rot_filp'

    if mode == 'scale_rot_flip' :
        scale = np.random.rand() * (max_scale - min_scale) + min_scale
    else :
        scale = 1.0

    if mode == 'scale_rot_filp':
        img = cv2.resize(img,(int(img.shape[1]*scale),int(img.shape[0]*scale)),cv2.INTER_LANCZOS4)

    shift_x = np.random.rand() * np.clip(int(scale * img.shape[1]) - crop_size, 0, 9999)
    shift_y = np.random.rand() * np.clip(int(scale * img.shape[0]) - crop_size, 0, 9999)

    crop_patch = img[int(shift_y):int(shift_y)+crop_size, int(shift_x):int(shift_x)+crop_size]
    if mode == 'leave_as_is' :
        k = 0
        crop_patch = crop_patch
    else :
        k = np.random.randint(8)
        crop_patch = np.rot90(crop_patch, k) if k < 4 else np.fliplr(np.rot90(crop_patch,k))

    return crop_patch, [shift_x, shift_y], k, scale
...
def random_augment(ims,
    ...
    shift_x = randx * np.clip(scale * im.shape[1] - crop_size, 0, 9999)
    shift_y = randy * np.clip(scale * im.shape[0] - crop_size, 0, 9999)
    shift_mat = np.array([[1, 0, - shift_x],
                          [0, 1, - shift_y],
                          [0, 0, 1]])
    #xxx = im.shape[1] * randx
    #yyy = im.shape[0] * randy
    ...
    if theta == 3*pi/2 or theta == 1*pi/2 :
        ...
        dst = np.clip(warpPerspective(im, transform_mat, (crop_size, crop_size), flags=INTER_CUBIC), 0, 1)
        gak = theta * 180 / pi
        rot_mat = cv2.getRotationMatrix2D((int(crop_size/2), int(crop_size/2)), gak, 1)
        patch = cv2.warpAffine(dst, rot_mat, (crop_size, crop_size))
    else :
        transform_mat = (shift_back_from_center
                         .dot(shift_mat)
                         .dot(shear_mat)
                         ...
                         .dot(scale_mat)
                         .dot(shift_to_center_mat))
        patch = np.clip(warpPerspective(im, transform_mat, (crop_size, crop_size), flags=INTER_CUBIC), 0, 1)

    # Apply transformation to image and return the transformed image clipped between 0-1
    return patch , [shift_x , shift_y], shift_back_from_center, shear_mat, rotation_mat, scale_mat, shift_to_center_mat, theta
...
def preprocess_kernels(kernels, conf):
    ...
    if kernels is not None:
        return [kernel_shift(loadmat(kernel)['Kernel'], sf) for kernel, sf in zip(kernels, conf.scale_factors)]
    else:
...
def prepare_result_dir(conf):
    ...
    if conf.create_results_dir:
        ...
        xxx = conf.result_path + '/Patches'
        os.makedirs(xxx)
        yyy = conf.result_path + '/output'
        os.makedirs(yyy)
